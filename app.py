...
st.caption("AI-Driven Predictive Analytics and Multi-Factor Operational Simulation Framework")
...
traffic_adjusted_time = business_adjusted_time * traffic_factor

# Weather
weather_factor = {"Clear":1.0, "Rain":1.15, "Heavy Rain":1.30}[weather_condition]
weather_adjusted_time = traffic_adjusted_time * weather_factor

# Weekend / Festival
day_factor = {"Weekday":1.0, "Weekend":1.10, "Festival":1.25}[day_type]
day_adjusted_time = weather_adjusted_time * day_factor

# Day / Night
time_factor = {"Day":1.0, "Night":1.08}[time_of_day]
final_operational_time = day_adjusted_time * time_factor
...
col2.metric("Business-Adjusted Time (mins)", f"{business_adjusted_time:.2f}")
...
if final_operational_time > sla_threshold:
    ...
    st.warning("Moderate Delay Risk – Monitor operations closely.")
else:
    st.success("Low Delay Risk – Delivery performance stable.")
...
folium.Marker([store_lat, store_lon], popup="Store Location",
              icon=folium.Icon(color="green")).add_to(m)

folium.Marker([customer_lat, customer_lon], popup="Customer Location",
